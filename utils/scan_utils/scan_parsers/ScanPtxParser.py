...
class ScanPtxParser(ScanParserABC):

    __supported_file_extension__ = [".ptx"]
    ...
    def get_point_dict(self, line):
        ...
        if self.rgb_color is False or len(line) == 4:
            rgb = [int(float(line[3] * 255))] * 3
        else:
            rgb = [int(r_g_b) for r_g_b in line[4:]]
    ...
    def parse(self, file_name=FILE_NAME):
        ...
        with open(file_name) as file:
            ...
            columns_count = int(file.readline())
            scan_position = []
            for _ in range(4):
                scan_position.append([float(numb) for numb in file.readline().strip().split()])
            transformation_matrix = []
            for _ in range(4):
                transformation_matrix.append([float(numb) for numb in file.readline().strip().split()])
            points = []
            for line in file:
