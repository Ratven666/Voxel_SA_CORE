...
class Poly2ModelPlotterMPL:

    def __init__(self, grid=10, alpha=1.0):
        self.model = None
        self._grid = grid
        self.__alpha = alpha
        self.__fig = None
    ...
    def plot(self, plane_model):
        ...
        for cell in self.model:
            if cell is None:
                continue
            x_min, y_min = -cell.voxel.step / 2, -cell.voxel.step / 2
            x_max, y_max = cell.voxel.step / 2, cell.voxel.step / 2
            X = np.arange(x_min, x_max + cell.voxel.step / self._grid, cell.voxel.step / self._grid)
            Y = np.arange(y_min, y_max + cell.voxel.step / self._grid, cell.voxel.step / self._grid)
            X, Y = np.meshgrid(X, Y)
            try:
                Z = (cell.a * X ** 2 +
                     cell.b * Y ** 2 +
            ...
            x_max, y_max = cell.voxel.X + cell.voxel.step, cell.voxel.Y + cell.voxel.step
            X = np.arange(x_min, x_max + cell.voxel.step / self._grid, cell.voxel.step / self._grid)
            Y = np.arange(y_min, y_max + cell.voxel.step / self._grid, cell.voxel.step / self._grid)
            X, Y = np.meshgrid(X, Y)
